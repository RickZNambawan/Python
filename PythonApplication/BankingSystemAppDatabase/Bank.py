...
class Bank(Frame):
    ...
    def createWidgets(self):
        ...
        self.addButton = Button(self, text="Add", width=15, command=self.addCustomer).grid(
            row=2, column=0, columnspan=2)
        self.deleteButton = Button(self, text="Delete", width=15, command=self.deleteCustomer).grid(
            row=2, column=1, columnspan=2, padx=5, pady=2)

        self.balanceButton = Button(self, text="Balance", width=15,
                                    command=self.viewBalance).grid(row=3, column=0, padx=5, pady=7)
        self.withdrawButton = Button(self, text="Withdraw", width=15, command=self.withdrawMoney).grid(
            row=3, column=1, padx=5, pady=7)
        self.depositButton = Button(self, text="Deposit", width=15,
                                    command=self.depositMoney).grid(row=3, column=2, padx=5, pady=7)

        self.resultBox = Text(self, width=40, height="8")
        self.resultBox.grid(row=4, column=0, columnspan=3)
        self.customersButton = Button(self, text="View Customer List", width=15, command=self.viewCustomerList).grid(
            row=5, column=0, columnspan=3, padx=5, pady=7)
    ...
    def addCustomer(self):
        ...
        if self.name.get() in self.bankDict:
            ...
            addMessage = tkinter.messagebox.askquestion(
                "Add Customer", "Do you want to add {}?".format(self.name.get()))
            if addMessage == "yes":
                c.execute("INSERT INTO Bank(name, money) VALUES (?, ?)",
                          (self.name.get(), self.money.get()))
                conn.commit()
                message = "{}, You've been successfully added in our Bank!\n".format(
                    self.name.get())
                message += "You've added amount of {:,} as your starting money!".format(
                    self.money.get())
                self.name.set("")
                self.money.set(0)
            else:
    ...
    def deleteCustomer(self):
        ...
        if self.name.get() == "":
            ...
            deleteMessage = tkinter.messagebox.askquestion(
                "Delete Customer", "Do you want to Delete {}?".format(self.name.get()))
            if deleteMessage == "yes":
                c.execute("DELETE FROM Bank WHERE name = ?", (self.name.get(),))
                conn.commit()
                message = "You've been successfully removed {} as your customer!".format(
                    self.name.get())
            else:
    ...
    def viewBalance(self):
        ...
        if self.name.get() == "":
            ...
            message = "Customer {}:\nYour remaining balance is: {:,}".format(
                self.name.get(), self.bankDict[self.name.get()])
            self.name.set("")
            self.money.set(0)
        self.resultBox.delete(0.0, END)
        self.resultBox.insert(0.0, message)
