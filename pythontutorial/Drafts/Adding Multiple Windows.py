...
class AddingBooks(Frame):
    def __init__(self, parent, controller):  # AddingBooks(root, MainApplication)
    ...
    #     c.execute("SELECT name FROM LIBRARY")
    #     for books in c:
    ...
    def addBooks(self):
        if self.name.get() in self.bookList:
            print("taena")
        else:
            self.bookList.append(self.name.get())
...
class RemovingBooks(Frame):
    def __init__(self, parent, controller):
        ...
        self.doNothing = Button(self, text="Wala lang", command=lambda: controller.showFrame(AddingBooks)).grid()
